...
FACTORY_OUTPUT_DIR = os.path.join(PROJECT_ROOT, "data")
...
class HotNewsScraper(Scraper):
    ...
    def run(self, start=None, end=None, max_articles=50) -> str:
        if start is None:
            start = datetime.now()
        if end is None:
            end = datetime(2020, 1, 1)
        print(f"[HotNews/Factory] Colectez articole în intervalul {start.date()} - {end.date()} (max {max_articles}) ...")
        new_articles = collect_hotnews_smart(start, end, max_articles=max_articles)
...
class Digi24Scraper(Scraper):
    ...
    def run(self, start=None, end=None, max_articles=50) -> str:
        ...
        print(f"[Digi24/Factory] Colectez articole în intervalul {start.date()} - {end.date()} (max {max_articles}) ...")
        new_articles = collect_digi24_smart(start, end, max_articles=max_articles)
        if isinstance(new_articles, dict):
            articles_list = new_articles.get("articles", [])
        else:
            articles_list = new_articles
...
class MediafaxScraper(Scraper):
    ...
    def run(self, start=None, end=None, max_articles=50) -> str:
        ...
        print(f"[Mediafax/Factory] Colectez articole în intervalul {start.date()} - {end.date()} (max {max_articles}) ...")
        new_articles = collect_mediafax_smart(start, end, max_articles=max_articles)
        if isinstance(new_articles, dict):
            articles_list = new_articles.get("articles", [])
        else:
            articles_list = new_articles
...
class Antena1Scraper(Scraper):
    ...
    def run(self, max_pages=1, max_articles=50) -> str:
        print(f"[Antena1/Factory] Colectez articole din primele {max_pages} pagini (max {max_articles}) ...")
        data = collect_antena1_stiri(max_pages=max_pages)
        articles_list = data.get("articles", [])[:max_articles]
